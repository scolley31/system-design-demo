...
# DynamoDB price：寫入被節流（on-demand 也可能短暫 throttle；crawler 批次寫入時留意）
resource "aws_cloudwatch_metric_alarm" "dynamo_write_throttle" {
  alarm_name          = "${var.project}-dynamo-write-throttle"
  comparison_operator = "GreaterThanThreshold"
  evaluation_periods  = 1
  threshold           = 0
  ...
  period              = 300
  statistic           = "Sum"
  dimensions          = { TableName = var.price_table_name }
  alarm_actions       = local.actions
  treat_missing_data  = "notBreaching"
}

resource "aws_cloudwatch_metric_alarm" "dynamo_read_throttle" {
  alarm_name          = "${var.project}-dynamo-read-throttle"
  comparison_operator = "GreaterThanThreshold"
  evaluation_periods  = 1
  threshold           = 0
  namespace           = "AWS/DynamoDB"
