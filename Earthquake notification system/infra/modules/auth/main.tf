...
locals {
  ...
  # 注意：GET /api/v1/stream（SSE）等公開端點
  # 不在此 → 落到 proxy 維持公開（SSE 由 device_id 帶識別，正式版走 APNs/FCM）。
  # （/api/v1/config/meta、/api/v1/earthquakes 亦維持公開）
  protected_routes = [
    "POST /api/v1/alerts/configuration",
    "GET /api/v1/alerts/configuration",
    "POST /api/v1/alerts/user_location",
