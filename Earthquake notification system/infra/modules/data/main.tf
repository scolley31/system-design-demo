...
resource "aws_elasticache_replication_group" "this" {
  ...
  description          = "${var.project} geo-index + Redis Streams + supersession cache"
  engine               = "redis"
...
# ---------- DynamoDB (user_location) ----------
# 位置是 high-write（≈ 5.5K writes/s，key=device_id）→ 用 DynamoDB on-demand。
# app 的 DynamoLocationStore 讀 LOCATION_TABLE（見 app/location_store.py）。
resource "aws_dynamodb_table" "user_location" {
  name         = "${var.project}-user-location"
  ...
  # TTL：讓 DynamoDB 自動過期舊位置（對齊 cell TTL）。
  # app 端寫 ttl_epoch（unix 秒）即被納管；未寫該屬性的 item 不會過期。
  ttl {
